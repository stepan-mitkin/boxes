(function () {
    ...
    function createElement(virtualElement) {
        ...
        if (virtualElement.type === "abs") {
            ...
            throw new Error("Unsupported virtual element type: " + virtualElement.type)
        }
        return element
    }
    ...
    function runCallbackCore(state, signal, callbackContext, privateContext) {
        ...
        if (signal.target) {
            ...
            if (result && result.setLocalState) {
    ...
    function buildWidgetTree(state, spec, path) {
        ...
        if (typeof builder !== "function") {
            throw new Error("builder \"" + builderName + "\" is not a function at: " + path)
        }
        var id
        if (spec.id) {
            id = spec.id
